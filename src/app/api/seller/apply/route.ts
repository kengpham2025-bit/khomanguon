...
export async function POST(req: Request) {
  ...
  if (u.seller_status === "rejected") {
    await db.prepare(`UPDATE users SET seller_status = 'none', updated_at = ? WHERE id = ?`).bind(now, s.sub).run();
  }
  ...
  await db
    ...
    .bind(id, s.sub, parsed.data.message ?? null, now)
    .run();

  await db.prepare(`UPDATE users SET seller_status = 'pending', updated_at = ? WHERE id = ?`).bind(now, s.sub).run();
